...
// the build tags (ie //go:build) enables a package to have more than one TestMain() function
// it stops go having name clashes
package data
...
func TestMain(m *testing.M) {
	...
	// capitalize on the backoff and retry functionality provided by dockertest
...
func TestUser_Insert(t *testing.T) {
	...
	if id == 0 {
		t.Error("0 returned after insert")
	}
}
...
func TestUser_PasswordMatches(t *testing.T) {
	...
	if matches {
		t.Error("password does match when it should not")
	}
}
...
func TestToken_Table(t *testing.T) {
	...
	if s != "tokens" {
		t.Error("wrong table name was returned for tokens")
	}
}
...
func TestToken_GetByToken(t *testing.T) {
	...
	if err != nil {
		t.Error("error getting token by PlainText: ", err)
	}

	_, err = models.Tokens.GetByToken("123")
	if err == nil {
		t.Error("expecting an error but did not receive one for a token that does not exist", err)
	}
}
...
var authData = []struct {
	...
	{name: "invalid", token: "abcdefghijklmnopqrstuvwxyz", email: "a@here.com", errExpected: true, message: "invalid token accepted as valid"},
	{name: "invalid_length", token: "abcdefghijklmnopqrstuvwxy", email: "a@here.com", errExpected: true, message: "token of wrong length accepted as valid"},
	{name: "no_user", token: "abcdefghijklmnopqrstuvwxyz", email: "a@here.com", errExpected: true, message: "no user, but token accepted as valid"},
	{name: "valid", token: "", email: "me@here.com", errExpected: false, message: "valid token reported as invalid"},
}
...
func TestToken_ExpiredToken(t *testing.T) {
	...
	token, err := models.Tokens.GenerateToken(u.ID, time.Hour*(-1))
	if err != nil {
		t.Error(err)
	}
...
func TestToken_BadHeader(t *testing.T) {
	...
	if err != nil {
		t.Error("failed to create a new user: ", err)
	}

	token, err := models.Tokens.GenerateToken(id, time.Hour)
	if err != nil {
		t.Error(err)
	}
...
func TestToken_ValidToken(t *testing.T) {
	...
	okay, _ = models.Tokens.ValidToken("abd")
	if okay {
